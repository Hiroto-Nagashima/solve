class RanksController < ApplicationController
  ...
  def index
    ...
    @monthly_scores = User.joins(:day_scores).where(day_scores: {created_at: Time.now.all_month}).where.not(users: {id: 2}).group(:id).order('sum(day_scores.day_score) desc').sample(5);
  end
end
