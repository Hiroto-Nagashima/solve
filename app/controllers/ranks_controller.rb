class RanksController < ApplicationController
  ...
  def index
    @weekly_tango_likes =Post.joins(:likes).limit(5).where(likes: {created_at: Time.now.all_day},genre: 0).group(:id).order(Arel.sql('count(likes.id) desc'));
    ...
    @monthly_likes = Post.joins(:likes).limit(5).where(likes: {created_at: Time.now.all_month}).group(:id).order(Arel.sql('count(likes.id) desc'));
