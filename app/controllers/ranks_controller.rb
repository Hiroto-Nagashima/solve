class RanksController < ApplicationController
  ...
  def index
    ...
    @weekly_scores = DayScore.joins(:user).limit(5).where(day_scores: {created_at: Time.now.all_week}).where.not(user_id: [1, 2]).order(sum_day_score: "desc").group(:user_id).select("day_scores.*, users.name as name, sum(day_score) as sum_day_score");
  end
end
